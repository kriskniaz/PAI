...
using NET.Kniaz.PAI.DES.Events;
...
namespace NET.Kniaz.PAI.DES
{
    public class Simulation
    {
        ...
        public void Execute()
        {
            string fname = Guid.NewGuid().ToString() + "_simulation.txt";
            var file = new StreamWriter(fname, true);
            string line = "time, land, air,total, runways ";
            file.WriteLine(line);
            ...
            while (_currentTime < MaxTime)
            {
                ...
                double diff = (_currentTime - StartTime).TotalMinutes;
                var runw = _runways.Select(val => val == true).ToArray().Count<bool>();
                line = diff.ToString() + "," + _airplanesOnLand.Count.ToString() + ',' 
                    + _waitingToLand.Count() + "," + _airplanes.Count.ToString() + "," + runw.ToString();
                file.WriteLine(line);
        ...
        public bool TryToLand(Airplane newPlane)
        {
            ...
            Console.WriteLine("Plane {0} put in queue to land", newPlane.Id);
            return false;
        }
    }
}
