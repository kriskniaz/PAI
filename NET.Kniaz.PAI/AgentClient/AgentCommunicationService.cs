...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "AgentCommunicationService" in both code and config file together.
    ...
    public class AgentCommunicationService : IAgentCommunicationService
    {
        ...
        public void Send(string from, string to, string message)
        {
            lock (_sycnRoot)
            {
                ...
                for (int i = _callbackChannels.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        Console.WriteLine("Detected Non-Open Callback Channel: {0}", _callbackChannels[i].GetHashCode()); _callbackChannels.RemoveAt(i);
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        Console.WriteLine("Pushed Updated List on Callback Channel: {0}",
                            _callbackChannels[i].GetHashCode());
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Service threw exception while communicating on Callback Channel: {0}", _callbackChannels[i].GetHashCode());
                        Console.WriteLine("Exception Type: {0} Description: {1}", ex.GetType(), ex.Message); _callbackChannels.RemoveAt(i);
                    }
                }
        ...
        public void Subscribe()
        {
            try
            {
                var callbackChannel = OperationContext.Current.GetCallbackChannel<IAgentCommunicationCallback>();
                lock (_sycnRoot)
                {
